...
class EventRecommender:
    def __init__(self, users_data: Dict, events_data: Dict):
...
def load_recommender():
    return EventRecommender(USERS_DATA, EVENTS_DATA)
...
def main():
    ...
    user_options = {f"{user['name']}": user['id'] for user in recommender.users}
    ...
    user = recommender.get_user_by_id(selected_user_id)
    ...
    if user:
        ...
        st.markdown("## 🎪 Recommended Events (>50% Match)")
        
        if recommendations:
            st.success(f"Found {len(recommendations)} events with >50% similarity match!")
